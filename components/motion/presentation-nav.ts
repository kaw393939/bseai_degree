...
export function goToSlide(oneBasedIndex: number): void {
  ...
  if (!target) return;
  // Land near the end of the sticky pin range so scroll-progress-driven
  // reveals (SceneCard, Reveal) have fully resolved to opacity 1. The
  // pin range is section height minus 100vh (the sticky stage); landing
  // ~10vh before its end keeps smooth-scroll overshoot inside the pin.
...
export function getActiveSlideIndex(): number {
  ...
  // A slide is "active" once we've scrolled past its start by at least
  // a bit — match the post-jump landing position by treating a slide as
  // active when its top is above a quarter of the viewport.
  let best = 0;
