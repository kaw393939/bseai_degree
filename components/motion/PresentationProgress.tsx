...
export function PresentationProgress({ slidesCount }: PresentationProgressProps) {
  ...
  const clampedWindowStart = Math.min(Math.max(reelPosition - viewportOffset, 0), maxWindowStart);
  ...
  const visibleDotIndices = dots.slice(renderedWindowStart, renderedWindowStart + visibleDots);
  ...
  return (
    ...
    >
      <div
        ...
        aria-label={`Slide ${activeIndex + 1} of ${slidesCount}. Click to jump to a slide.`}
      >
        {isEditing ? (
          <input
            ref={inputRef}
        ...
        <span className="presentation-progress__count-total">{String(slidesCount).padStart(2, "0")}</span>
      </div>
