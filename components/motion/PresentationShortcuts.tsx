...
export function PresentationShortcuts({ slidesCount }: PresentationShortcutsProps) {
  ...
  const submitPalette = () => {
    ...
    if (lower === "first" || lower === "home" || lower === "top") {
      firstSlide();
    } else if (lower === "last" || lower === "end") {
      lastSlide();
    } else if (lower === "next") {
      nextSlide();
    } else if (lower === "prev" || lower === "previous" || lower === "back") {
      prevSlide();
    ...
    <>
      {numberBuffer && (
        <div className="presentation-number-buffer" aria-live="polite">
          Jump to slide <strong>{numberBuffer}</strong> <span>— press Enter</span>
        </div>
      )}
      ...
      {overlay === "palette" && (
        ...
        >
          <div className="presentation-palette">
            <label className="presentation-palette__label" htmlFor="presentation-palette-input">
              Jump to slide
            </label>
            <input
              id="presentation-palette-input"
              ref={paletteInputRef}
              ...
              placeholder={`Slide number (1–${slidesCount}), or: first, last, next, prev, help`}
              autoComplete="off"
              spellCheck={false}
            />
        ...
        >
          <div className="presentation-help">
            ...
            <p className="presentation-help__footnote">
              You can also click the slide counter at the right edge to edit it inline.
            </p>
          </div>
        </div>
      )}
